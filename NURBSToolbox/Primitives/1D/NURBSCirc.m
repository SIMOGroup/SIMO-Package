function Curv = NURBSCirc(radius, center, sang, eang)
% 
...
CtrlPts = zeros(4,2*narcs+1);   % nurbs control points of arc
xx = vecrotz(sang + dsweep);
CtrlPts(:,1:3) = xx*ctrlpt;     % rotate to start angle
xx = vecrotz(2*dsweep);
for n = 2:narcs
   m = 2*n+[0 1];
   CtrlPts(:,m) = xx*CtrlPts(:,m-2);
end
