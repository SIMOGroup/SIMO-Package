function finNURBS = NURBSTForm(iniNURBS, tmat)
%
% NRBTFORM: Apply transformation matrix to the NURBS.
%
% Calling Sequence:
%
%   tnurbs = NURBSTForm(nurbs,tmatrix);
%
% INPUT:
%
...
if numel(iniNURBS.KntVect) > 1
    if numel(iniNURBS.KntVect) == 2
        % NURBS is a surface
        [dim,nu,nv] = size(iniNURBS.CtrlPts4D);
        CtrlPts = reshape(tmat*reshape(iniNURBS.CtrlPts4D,dim,nu*nv),[dim nu nv]);
        finNURBS = CreateNURBS(iniNURBS.KntVect, CtrlPts);
    elseif numel(iniNURBS.KntVect) == 3
        % NURBS is a volume
        [dim,nu,nv,nw] = size(iniNURBS.CtrlPts4D);
        CtrlPts = reshape(tmat*reshape(iniNURBS.CtrlPts4D,dim,nu*nv*nw),[dim nu nv nw]);
        finNURBS = CreateNURBS(iniNURBS.KntVect, CtrlPts);
    end
else
    % NURBS is a curve
    CtrlPts = tmat*iniNURBS.CtrlPts4D;
    finNURBS = CreateNURBS(iniNURBS.KntVect, CtrlPts);
end
